...
export default function NewTaskPage() {
  ...
  const statusOptions = useMemo(() => {
    if (initialStatus && !DEFAULTS.includes(initialStatus)) {
      return [initialStatus, ...DEFAULTS];
    }
    return DEFAULTS;
  }, [initialStatus]);
    ...
    <div className="px-4 py-4">
      ...
      <div className="mb-8 flex items-start justify-between">
        <div>
          ...
          <h1 className="text-2xl font-black tracking-tighter text-dark">New Task</h1>
          <p className="mt-1 text-sm font-bold text-zinc-500">Add a new task to the board.</p>
        </div>
        <Link
          href={`/dashboard/projects/${projectId}`}
