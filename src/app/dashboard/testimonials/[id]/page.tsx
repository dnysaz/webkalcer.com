...
export default async function EditTestimonialPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      <p className="mt-1 mb-10 text-sm font-bold text-zinc-400">Edit testimonial data.</p>
        ...
        <div className="rounded-2xl border-2 border-zinc-200 bg-white p-6">
          ...
          <div className="mt-4 space-y-4">
            <div>
              ...
              <input name="name" defaultValue={t.name} required className="w-full rounded-xl border-2 border-zinc-200 px-4 py-3 text-sm font-bold outline-none transition focus:border-pink" />
            </div>
            <div>
              <label className="mb-1 block text-sm font-bold text-zinc-600">Role / Job</label>
              <input name="role" defaultValue={t.role} required className="w-full rounded-xl border-2 border-zinc-200 px-4 py-3 text-sm font-bold outline-none transition focus:border-pink" />
            </div>
            <div>
              <label className="mb-1 block text-sm font-bold text-zinc-600">Quote</label>
              <textarea name="quote" defaultValue={t.quote} required rows={3} className="w-full rounded-xl border-2 border-zinc-200 px-4 py-3 text-sm font-bold outline-none transition focus:border-pink" />
            </div>
            <div className="flex gap-4">
              <div className="flex-1">
                <label className="mb-1 block text-sm font-bold text-zinc-600">Sort Order</label>
                <input name="sort_order" type="number" defaultValue={t.sort_order} className="w-full rounded-xl border-2 border-zinc-200 px-4 py-3 text-sm font-bold outline-none transition focus:border-pink" />
              </div>
