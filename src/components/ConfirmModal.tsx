  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="mx-4 w-full max-w-sm rounded-3xl border-2 border-zinc-700 bg-dark p-6 shadow-2xl">
        ...
        <p className="mt-2 text-sm font-bold leading-relaxed text-zinc-100">{description}</p>
        <div className="mt-6 flex gap-3">
          <button
            onClick={onCancel}
            disabled={loading}
            className="flex-1 rounded-full border-2 border-zinc-500 px-4 py-2.5 text-sm font-bold text-zinc-100 transition hover:border-zinc-300 disabled:opacity-50"
          >
            Cancel
          </button>
          <button
